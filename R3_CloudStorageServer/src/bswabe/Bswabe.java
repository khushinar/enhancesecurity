...
public class Bswabe {
    ...
    public static BswabeElementBoolean dec(BswabePub pub, BswabePrv prv,
        ...
        if(Bswabe.settimeflag){
           System.err.println("cannot decrypt, Session expired");
            beb.e = null;
            beb.b = false;
            Bswabe.settimeflag=false;
            return beb;
        }
    ...
    private static void checkSatisfy(BswabePolicy p, BswabePrv prv) {
        ...
        if (p.children == null || p.children.length == 0) {
            for (i = 0; i < prv.comps.size(); i++) {
                ...
                if (prvAttr.contains("Time") && p.attr.contains("Time")) {
                    String usertime[] = prvAttr.trim().split(":");
                    String policytime[] = p.attr.trim().split(":");
                    int policyt = Integer.parseInt(policytime[1]);
                    int usert = Integer.parseInt(usertime[1]);
                    
                    System.out.println("Policy Time:"+policyt);
                    System.out.println("user time:"+usert);
                   if(usert > policyt){
                     
                      Bswabe.settimeflag=true;
